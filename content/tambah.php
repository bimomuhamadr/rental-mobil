...
if($_GET['tambah']=="user") {
	...
	if(isset ($_POST['simpan-user'])){
		...
		if($rows>0){
			...
			mysql_query("INSERT INTO `t_user`(`nik_ktp`,`no_sim`,`nama_user`,`email_user`,`no_hp`,`alamat`,`password`,`level_user`,`akses`) VALUES ('$nik_ktp','$no_sim','$nama_user','$email_user','$no_hp','$alamat',MD5('$password'),'customer','on')");
    ...
    if (!empty($_FILES["link_gambar"]["tmp_name"])){
        ...
        if($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/gif" || $jenis_gambar=="image/png"){      
            ...
            if (move_uploaded_file($_FILES['link_gambar']['tmp_name'], $gambar)) {
                ...
                mysql_query("UPDATE `t_transaksi` SET `status_bayar`='$status_bayar' WHERE `id_transaksi`='$id_transaksi'");
